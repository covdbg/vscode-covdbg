...
export async function showMenu(ctx: MenuContext, actions: MenuActions): Promise<void> {
    ...
    if (ctx.isLoaded && ctx.activeCovdbPath) {
        ...
        items.push({ label: 'Coverage Database', kind: vscode.QuickPickItemKind.Separator, action: '' });
        items.push({
        ...
        if (ctx.activeFileSummary) {
            ...
            items.push({ label: 'Current File', kind: vscode.QuickPickItemKind.Separator, action: '' });
            const display = vscode.workspace.workspaceFolders
                ? vscode.workspace.asRelativePath(s.filePath) : path.basename(s.filePath);
            items.push({
    ...
    if (!picked || !picked.action || picked.action === 'noop') { return; }
    ...
    switch (picked.action) {
        case 'toggle':
            ...
            break;
        case 'render-mode':
            if (picked.mode) { actions.setRenderMode(picked.mode); }
            break;
        case 'browse':
            actions.browse();
            break;
...
async function showDatabaseSwitcher(ctx: MenuContext, actions: MenuActions): Promise<void> {
    interface DbItem extends vscode.QuickPickItem { covdbPath?: string; action?: string; }
    ...
    for (const uri of ctx.availableCovdbFiles) {
        const isCurrent = ctx.activeCovdbPath && path.normalize(uri.fsPath).toLowerCase() === path.normalize(ctx.activeCovdbPath).toLowerCase();
        const display = vscode.workspace.workspaceFolders
    ...
    switch (picked.action) {
        case 'close':
            ...
            break;
        case 'switch':
            if (picked.covdbPath) { actions.switchDatabase(picked.covdbPath); }
            break;
        case 'browse':
            actions.configure();
            break;
...
export async function showFileBrowser(fileIndex: Map<string, CovdbFileSummary>): Promise<void> {
    ...
    interface FileItem extends vscode.QuickPickItem { filePath: string; }

    const items: FileItem[] = Array.from(fileIndex.values())
        .sort((a, b) => a.filePath.localeCompare(b.filePath))
        .map(s => {
            const pct = s.coveragePercent.toFixed(1);
            const icon = s.coveragePercent >= 80 ? '$(check)'
                : s.coveragePercent >= 50 ? '$(warning)' : '$(error)';
            const display = vscode.workspace.workspaceFolders
...
function computeOverallStats(fileIndex: Map<string, CovdbFileSummary>): { covered: number; total: number } {
